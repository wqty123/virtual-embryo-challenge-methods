...
1. **T1**：`pseudobulk shift（全局漂移）` 48.13 → damp 网格 48.85 → `OT 混合（WOT 耦合+类型转移重采样）` 47.52 → `Markov 通量`（证伪 44.19）→ `程序混合`（vario 崩 47.34）→ `momentum mask` 49.49 → `mm 库大小归一化` 49.93 → `vs 虚拟状态` 50.67 → **`kp 成熟度加权混合管线`（kp3_w 52.18 → kp7_m_b050 53.67，当前最佳）**
2. **T2 embryo**：`v1（kNN 配对 log-linear 插值）` 59.0 → `mix_brkt` 59.07 → `bgm` 62.94 → `gps33 高斯过程` 63.19 → **`cpf015r5k 配对条件流` 65.31（当前最佳）**
...
5. **T3**：`transfer 全局Δ`（证伪 40.98）→ `cardiac 限制`（证伪 44.85）→ `文献先验`（平地板 45.80）→ `mix 混合 KO` 64.60 → `cmp 精准 KO 效应` 64.93 → `cmpw` 65.49 → `kb 成熟度加权` 66.36 → **`kbb 组合` 66.61（当前最佳）**
